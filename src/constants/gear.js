...
export function createRespectSchematic(rarity = "soldier") {
  const PTS = { soldier: 200, veteran: 500, champion: 1000 };
  return {
    instanceId: `rs_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,
    type:       "respectSchematic",
    rarity,
    points:     PTS[rarity] ?? 200,
    icon:       rarity === "champion" ? "🏆" : rarity === "veteran" ? "⭐" : "📜",
    n:          `${rarity.charAt(0).toUpperCase() + rarity.slice(1)} Respect Schematic`,
  };
}
...
export function rollFullPull(alignFactions, playerAlignment, pityCounters) {
  // Slot 1: 98% respect schematic, 2% full commander pull (doc-specified curve)
  const slot1IsCmd = Math.random() < 0.02;
  let slot1;
  if (slot1IsCmd) {
    // Use existing rollGacha logic — pass as single pull
    // (We return a flag so Game.jsx can run the full rollGacha+pity logic)
    slot1 = { type: "commander" };
  } else {
    // Respect schematic — rarity weighted
    const r = Math.random();
    // Schematic rarity: 88% soldier, 8% veteran, 2% champion (doc-specified curve)
    const scRarity = r < 0.02 ? "champion" : r < 0.10 ? "veteran" : "soldier";
    slot1 = createRespectSchematic(scRarity);
  }
  ...
  if (s3r < 0.01) {
    ...
    const r = Math.random();
    // Schematic rarity: 88% soldier, 8% veteran, 2% champion
    const scRarity = r < 0.02 ? "champion" : r < 0.10 ? "veteran" : "soldier";
    slot3 = createRespectSchematic(scRarity);
  } else {
