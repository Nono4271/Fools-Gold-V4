...
const BRANCH_SKILL_MAP = {
  ...
  leader: [
    ...
    { main:"forced_march",     sides:["siege_mastery",      "shield_order"]      },
  ],
};

// ── Public helpers ────────────────────────────────────────────────────────────

export function getBranchMainSkill(cls, branchIndex) {
...
export function getBranchSideSkills(cls, branchIndex) {
  const branches = BRANCH_SKILL_MAP[cls] ?? BRANCH_SKILL_MAP.attacker;
  const branch   = branches[branchIndex % branches.length];
...
export const SKILL_TREES = {
  combat:  { branches:4, cls:"attacker" },
  defense: { branches:4, cls:"defender" },
  tactics: { branches:4, cls:"support"  },
  command: { branches:4, cls:"leader"   },
};

export function getCommanderTrees(cls) {
  return [{ attacker:"combat", defender:"defense", support:"tactics", leader:"command" }[cls] ?? "combat"];
}

// ── Backwards-compat stubs ────────────────────────────────────────────────────
export const MAIN_SKILLS       = ALL_SKILLS;
export const SIDE_SKILLS       = ALL_SKILLS;
