...
// Polys in design coordinate space (x: 0–700, y: 0–700) — matches WorldMap.jsx
// Shattered Shallows is split into 3 sub-polygons to avoid self-intersection around the Holy Grail notch.
// All three share the 'shatteredShallows' region key for hit-testing purposes.
export const POLYS = {
...
// Internal keys that are sub-parts of shatteredShallows — map them to the parent region
...
export function tileRegion(c, r) {
  for (const [key, poly] of Object.entries(POLYS)) {
    if (pointInPoly(c, r, poly)) {
      // Sub-parts of shallows resolve to the parent region
      const regionKey = SHALLOWS_PARTS.has(key) ? 'shatteredShallows' : key;
      return REGION_LIST.find(rg => rg.key === regionKey);
    }
  }
  return null;
}
