...
import { HQP, AI_HQ_KEY, WIN_KEY, RKEYS, RSS, POWER_DEFS, SIEGE_BASE, calcSiegePower } from "./constants/map.js";
...
export default function RiseToWar() {
  ...
  const [cmds,   setCmds]    = useState(() => HDEFS.filter(h => h.faction==="pirates" && h.rarity==="veteran").slice(0,2).map((h,i) => ({...h, uid:`p${i}`, owner:"player", troops:0, troopType:null, tk:`${HQP.player.c},${HQP.player.r}`, lvl:5, xp:0, respectPoints:0, respectLevel:0, skillPoints:{}, unspentSkillPoints:5, gear:{helmet:null,armor:null,bracers:null,accessory:null}})));
  const [coll,   setColl]    = useState(() => HDEFS.filter(h => h.faction==="pirates" && h.rarity==="veteran").slice(0,2));
  ...
  // ── Hooks ──
  useResources({ screen, tiles, setRss });
      ...
      if (expired.length > 0) {
        setTiles(p => {
          ...
          expired.forEach(key => {
            ...
            next[key] = { ...t, owner:null, garrison:pd?pd.troops:50, siege:t.siegeMax??SIEGE_BASE, siegeMax:t.siegeMax??SIEGE_BASE, garrisonDefeated:false, resetAt:null, defCmd:pd?{lvl:pd.cmdLvl,troops:pd.troops,troopType:TROOP_KEYS[0],atk:80,spd:40}:null };
          });
          return next;
        });
  ...
  const recallStationary = useCallback((uid) => {
    setCmds(prev => {
      ...
      const stepMs = marchStepMs(effectiveMarchSpd(cmd.spd||60, cmd.troopType));
      return prev.map(c => c.uid===uid ? { ...c, march:{ type:"move", path, step:0, dest:hqKey, origin:cmd.tk, stepMs, lastStepTime:Date.now() } } : c);
    });
  }, []);
  ...
  const pull = useCallback((n) => {
    ...
    for (let p = 0; p < n; p++) {
      const { slot1, slot2, slot3 } = rollFullPull(alignFactions, playerAlignKey, newPity);
      ...
      slots.forEach(slot => {
        if (slot.type === "commander") {
          // Use full-pull rarity rates (soldier 50%/veteran 35%/champion 15% within cmd slots)
          // This gives approx. soldier ~1%, veteran ~0.7%, champion ~0.3% of total pulls
          const forcedRarity = rollFullPullCmdRarity();
          // Temporarily bias pity to guarantee the rolled rarity
          ...
          // Only update real pity for the triggered tier
    ...
    // Add gear to inventory (always goes to inventory, never auto-consumed)
    if (newGear.length) setGearInventory(prev => [...prev, ...newGear]);

    // Add respect schematics to inventory
    if (newSchematics.length) setRespectSchematics(prev => [...prev, ...newSchematics]);

    // Process commander results
  ...
  return (
    <div style={{width:"100vw",height:"100vh",position:"relative",overflow:"hidden",background:"#0e1014",userSelect:"none",touchAction:"none"}}>
      ...
      <HUD facName={facName} pKeys={pKeys} rss={rss} gems={gems} setCmdScreenOpen={setCmdScreenOpen} />
