...
export default function RiseToWar() {
  ...
  // panSt/zoom are ONLY used by Minimap, WorldMap, and the zoom % label.
  // All hot-path code (tile clicks, floaties, worker callbacks) reads the refs
  // directly so Pixi pan/zoom gestures never trigger a full Game re-render.
  const [displayPanZoom, setDisplayPanZoom] = useState({ pan: { x:4, y:4 }, zoom: 1.25 });
  const panSt = displayPanZoom.pan;
  const zoom  = displayPanZoom.zoom;

  // Call this only when UI consumers actually need to repaint (zoom button,
  // MapRenderer onPanChange throttled to ~200ms, minimap open).
  ...
  useEffect(() => {
    ...
    worker.onmessage = (e) => {
      ...
      if (type === "done") {
        ...
        setTiles(rawMap);
        // Push correct initial pan/zoom into Pixi now that map is ready
        // (MapRenderer mounted with default pan {4,4}; teleport corrects it)
  ...
  const { initPathfinding, findPath } = usePathfinding();
  ...
  useEffect(() => {
    if (screen !== "game") return;
    const id = setInterval(() => {
      mapRendererRef.current?.redrawOverlays?.();
    }, 200);
    return () => clearInterval(id);
  }, [screen]);
