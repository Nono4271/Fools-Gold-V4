...
export default function RiseToWar() {
  ...
  // Write panRef immediately so MapRenderer is correct the instant WorldMap closes.
  // Defer setPanSt to a rAF so the React state update fires after WorldMap unmounts,
  // preventing a camera-stutter render while the overlay is still visible.
  const teleportTo = useCallback((tc, tr) => {
    ...
    const py = -cy * z + window.innerHeight / 2;
    panRef.current = { x: px, y: py };
    requestAnimationFrame(() => setPanSt({ x: px, y: py }));
  }, []);
  ...
  const handleZoomChange = useCallback((newZoom) => {
    ...
    setZoom(newZoom);
    // Re-clamp the current pan to the new zoom level (mirrors clampPan in MapRenderer)
    const scaledW = ISO_W * newZoom;
    const scaledH = ISO_H * newZoom;
    const cx = Math.min(60, Math.max(-(scaledW - window.innerWidth  + 60), panRef.current.x));
    const cy = Math.min(60, Math.max(-(scaledH - (window.innerHeight - 40) + 60), panRef.current.y));
  ...
  }, [tileVersion, floaty, gearInventory]);
  ...
  const onTileClick = useCallback((k, e) => {
    ...
    if (tile.isHQ && tile.owner==="player") {
      // Show "Enter HQ" popup on the tile instead of opening menu directly
      const elev2 = 14;
      const { cx: hqCx, cy: hqCy } = isoXY(tile.c, tile.r);
      const sX = hqCx * zoom + panRef.current.x;
      const sY = (hqCy - elev2) * zoom + panRef.current.y + 38;
    ...
    setMode("view"); setAtkKey(null); setPick(null); setMvCmd(null); setReinCmd(null);
  }, [tileVersion, mode, mvCmd, startMarch, zoom, floaty]);
