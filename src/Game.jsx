...
import { isoXY, TW, TH } from "./constants/geometry.js";
...
export default function RiseToWar() {
  ...
  useEffect(() => {
    ...
    setTiles(p => {
      // Update all AI HQ tiles (there are now up to 5)
      const aiHqTiles = Object.values(p).filter(t => t.isHQ && t.owner === "ai");
      if (!aiHqTiles.length) return p;
      const next = { ...p };
      aiHqTiles.forEach(tile => {
        const newSiege = Math.min(tile.siege ?? newAiSiegeMax, newAiSiegeMax);
        next[tile.k] = { ...tile, siegeMax: newAiSiegeMax, siege: newSiege };
      });
      return next;
    });
  ...
  useEffect(() => {
    if (screen === "game" && tiles !== null && typeof tiles === "object" && tiles.__ready) {
      setMapReady(true);
    }
  }, [tiles, screen]);
  ...
  // ── Computed ──
  // pKeys tracks player-owned tile keys. Derived from tiles but only recomputed
  // when tiles actually changes — expensive at 490k keys so kept as a separate
  // maintained Set rather than recomputed from scratch each time.
  const pKeys = useMemo(() => new Set(
    Object.keys(tiles).filter(k => tiles[k]?.owner === "player")
  ), [tiles]);
