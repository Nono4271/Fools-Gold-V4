...
// ── Single branch SVG canvas ──────────────────────────────────────────────────
function BranchCanvas({
  faction, color, accent, locked,
  mainSkill, mainLvl,
  sideSkills, sideLvls, sideCaps,
  liveUnspent, onNodeClick, selectedKey,
}) {
  const W = 320, H = 300;
  const spineX = W / 2;
  // 1 main node centred, 2 side nodes each side
  const mainY = H / 2;
  const mainSize = 58;
  const sideSize = 38;
  const sideXL = spineX - 100;
  const sideXR = spineX + 100;
  const sideYs = [mainY - 44, mainY + 44];
  ...
  return (
    <svg width={W} height={H} style={{ display: "block", overflow: "visible" }}>
      {/* background glow */}
      <ellipse cx={spineX} cy={mainY} rx={130} ry={110}
        fill={color} fillOpacity={locked ? 0 : 0.04} />

      {/* connectors: main → left sides */}
      {sideYs.map((sy, i) => (
        <Connector key={`cl${i}`}
          x1={spineX - mainSize / 2} y1={mainY + (i === 0 ? -mainSize * 0.15 : mainSize * 0.15)}
          x2={sideXL + sideSize / 2} y2={sy}
          color={color}
          lit={!locked && mainFilled}
          dashed={true}
        />
      ))}

      {/* connectors: main → right sides */}
      {sideYs.map((sy, i) => (
        <Connector key={`cr${i}`}
          x1={spineX + mainSize / 2} y1={mainY + (i === 0 ? -mainSize * 0.15 : mainSize * 0.15)}
          x2={sideXR - sideSize / 2} y2={sy}
          color={color}
          lit={!locked && mainFilled}
          dashed={true}
        />
      ...
      {/* left side nodes */}
      {sideSkills.slice(0, 1).map((sk, i) => {
        const lvl = sideLvls[0];
        const cap = sideCaps[0];
        const gateLocked = lvl >= cap;
        const nodeLocked = locked || !mainFilled;
        const sel = selectedKey === sk.key;
        return (
          <g key={`sl${i}`}
            transform={`translate(${sideXL - sideSize / 2},${sideYs[0] - sideSize / 2})`}
            onClick={() => !nodeLocked && onNodeClick(sk, false, gateLocked && lvl < 5)}
            style={{ cursor: nodeLocked ? "default" : "pointer" }}>
            <FactionNode faction={faction} size={sideSize}
              filled={lvl > 0} color={color} accent={accent}
              locked={nodeLocked} isMain={false} selected={sel} />
            {!nodeLocked && (
              <>
                <text x={sideSize / 2} y={-6} textAnchor="middle"
                  fontSize={6.5} fill={lvl > 0 ? accent : "#4a3a28"}
                  fontFamily="'Cinzel',serif">{sk.name?.split(" ")[0]}</text>
                <text x={sideSize / 2} y={sideSize + 10} textAnchor="middle"
                  fontSize={7} fill={lvl > 0 ? color : "#3a2a18"}
                  fontFamily="'Cinzel',serif">{lvl}/5</text>
                {gateLocked && lvl < 5 && (
                  <text x={sideSize / 2} y={sideSize + 20} textAnchor="middle"
                    fontSize={6} fill="#c07830" fontFamily="'Cinzel',serif">🔒</text>
                )}
              </>
            )}
          </g>
      ...
      {/* right side node 0 */}
      {sideSkills.slice(1, 2).map((sk, i) => {
        const lvl = sideLvls[1];
        const cap = sideCaps[1];
        const gateLocked = lvl >= cap;
        const nodeLocked = locked || !mainFilled;
        const sel = selectedKey === sk.key;
        return (
          <g key={`sr${i}`}
            transform={`translate(${sideXR - sideSize / 2},${sideYs[0] - sideSize / 2})`}
            onClick={() => !nodeLocked && onNodeClick(sk, false, gateLocked && lvl < 5)}
            style={{ cursor: nodeLocked ? "default" : "pointer" }}>
            <FactionNode faction={faction} size={sideSize}
              filled={lvl > 0} color={color} accent={accent}
      ...
      {/* main node */}
      <g transform={`translate(${spineX - mainSize / 2},${mainY - mainSize / 2})`}
        onClick={() => !locked && onNodeClick(mainSkill, true, false)}
        style={{ cursor: locked ? "default" : "pointer" }}>
        <FactionNode faction={faction} size={mainSize}
          filled={mainFilled} color={color} accent={accent}
          locked={locked} isMain={true} selected={selectedKey === mainSkill.key} />
        {!locked && (
          <>
            <text x={mainSize / 2} y={mainSize / 2 + 7} textAnchor="middle"
              fontSize={18} style={{ pointerEvents: "none" }}>{mainSkill.icon}</text>
            <text x={mainSize / 2} y={mainSize + 13} textAnchor="middle"
              fontSize={7.5} fill={mainFilled ? accent : "#6a5a3a"}
              fontFamily="'Cinzel',serif" fontWeight={mainFilled ? "700" : "400"}>
              {mainSkill.name}
            </text>
            <text x={mainSize / 2} y={mainSize + 24} textAnchor="middle"
      ...
      {locked && (
        <g>
          <rect x={0} y={0} width={W} height={H} fill="rgba(0,0,0,.5)" rx={8} />
        </g>
      )}
    </svg>
  );
          ...
          {treeEntries.map(({ treeKey, label, unlocksAt, tag, branchIdx }, idx) => {
            ...
            return (
              <div key={`tree_${idx}`} style={{
                background: locked ? "rgba(255,255,255,.01)" : "rgba(255,255,255,.025)",
                border: `1px solid ${locked ? "#1c1c1c" : (isLegacy ? "#6a5a3040" : fColor + "35")}`,
                ...
                <div style={{
                  padding: "8px 12px",
                  background: locked ? "transparent" : `linear-gradient(90deg, ${isLegacy ? "#6a5a3010" : fColor + "10"}, transparent)`,
                  borderBottom: `1px solid ${locked ? "#181818" : (isLegacy ? "#6a5a3018" : fColor + "20")}`,
                  display: "flex", alignItems: "center", gap: 8,
                }}>
                  <span style={{ fontSize: 15 }}>{tree.icon}</span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontFamily: "'Cinzel',serif", fontSize: 9, fontWeight: 700,
                      color: locked ? "#333" : "#e0d0c0",
                      overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{label}</div>
                    <div style={{ fontSize: 7, color: locked ? "#252525" : "#5a4a3a",
                      fontFamily: "'Crimson Pro',serif", marginTop: 1 }}>{tree.desc}</div>
                  </div>
                  <div style={{ flexShrink: 0 }}>
                    {locked
                      ? <div style={{ padding: "2px 6px", borderRadius: 3, border: "1px solid #cc303040",
                          fontSize: 6, color: "#cc3030", fontFamily: "'Cinzel',serif" }}>🔒 R{unlocksAt}</div>
                      : <div style={{ fontSize: 7, color: totalInBranch > 0 ? "#3daa60" : "#2a3a20",
                          fontFamily: "'Cinzel',serif" }}>{totalInBranch > 0 ? `${totalInBranch}pt` : tag}</div>
                    }
                  </div>
                </div>
