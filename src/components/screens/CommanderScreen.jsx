  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: "100%" }}>
      ...
      {previewGear && (() => {
        ...
        const pgRarityMult = pgR.statMult ?? 1;
        const pgPVal = Math.round((pgPBase[pg.primaryStat] ?? 7) * pgRarityMult * (1 + (pg.stars ?? 0) * 0.12));
            ...
            <div onClick={e => e.stopPropagation()} style={{
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center",
                ...
                <span style={{ fontSize:9, color:"#6a5a3a", fontFamily:"'Cinzel',serif" }}>
                  {pgStatIcons[pg.primaryStat]} {({ ATK:"Attack", FOC:"Focus", SPD:"Speed" })[pg.primaryStat] ?? pg.primaryStat}
                </span>
...
export default function CommanderScreen({ cmds, bldgs, gearInventory, setGearInventory, respectSchematics, setCmds, onSchematicUsed, onClose, initialUid, gems, setGems }) {
  ...
  const [filterSubspecies, setFilterSubspecies] = useState(null); // subspecies string | null
  ...
  const filtered = allPlayer.filter(c => {
    ...
    if (filterSubspecies && c.subspecies !== filterSubspecies) return false;
    return true;
  }).sort((a, b) => {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 5, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <Chip label="All" active={!filterAlignment && !filterClass && !filterSubspecies}
            onClick={() => { setFilterAlignment(null); setFilterClass(null); setFilterSubspecies(null); }} />
        ...
        <div style={{ display: "flex", gap: 5, flexWrap: "wrap", alignItems: "center" }}>
          <span style={{ fontSize: 7, color: "#3a2e18", fontFamily: "'Cinzel',serif",
            letterSpacing: ".06em", flexShrink: 0 }}>RANK</span>
          <Chip label="All" active={!filterSubspecies}
            onClick={() => setFilterSubspecies(null)} />
          {Object.entries(SUBSPECIES).flatMap(([faction, tiers]) =>
            Object.values(tiers).map(label => ({ label, faction }))
          ).filter((item, idx, arr) =>
            arr.findIndex(x => x.label === item.label) === idx
          ).map(({ label }) => (
            <Chip key={label} label={label}
              active={filterSubspecies === label}
              onClick={() => setFilterSubspecies(filterSubspecies === label ? null : label)} />
          ))}
        </div>
      </div>
