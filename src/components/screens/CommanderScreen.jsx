...
function SkillNode({ skillKey, skillDef, level, maxLevel, unspent, rarityColor, onLevelUp, isMain }) {
  const [expanded, setExpanded] = useState(false);
  const canLevelUp = unspent > 0 && level < maxLevel;
        ...
        <div style={{
          ...
          {level < maxLevel ? (
            <button
              ...
              style={{
                ...
                background: canLevelUp
                  ...
                  : "rgba(255,255,255,.02)",
                border: `1px solid ${canLevelUp ? rarityColor + "60" : "#1e1810"}`,
                borderRadius: 4, cursor: canLevelUp ? "pointer" : "not-allowed",
                fontFamily: "'Cinzel',serif", fontSize: 9, fontWeight: 700,
                color: canLevelUp ? rarityColor : "#2e2418",
            ...
            >
              ...
              {canLevelUp ? `Level Up (1 point)` : unspent === 0 ? "No points available" : "Max level reached"}
            </button>
          ) : (
...
function CommanderDetail({ cmd, bldgs, gearInventory, setGearInventory, respectSchematics, setCmds, onSchematicUsed }) {
  ...
  return (
    <>
      {showSkills && <SkillTreeOverlay cmd={cmd} setCmds={setCmds} onClose={() => setShowSkills(false)} />}
