...
function TroopBar({ start, end, wounded, isEnemy }) {
  ...
  const wCapped = Math.min(wounded ?? 0, Math.max(0, start - end));
  const killed  = Math.max(0, start - end - wCapped);
  const pctR = Math.round((end    / safe) * 100);
  const pctW = Math.round((wCapped / safe) * 100);
  const pctK = Math.max(0, 100 - pctR - pctW);
  return (
    <div style={{ height:5, background:"#0c0905", borderRadius:3, overflow:"hidden", display:"flex" }}>
      <div style={{ width:`${pctR}%`, background:"linear-gradient(90deg,#2255cc,#4488ff)", transition:"width .4s" }} />
      {!isEnemy && pctW > 0 && (
        <div style={{ width:`${pctW}%`, background:"linear-gradient(90deg,#1a8040,#3daa60)", transition:"width .4s" }} />
      )}
      {pctK > 0 && (
...
function BarLegend({ start, end, wounded, isEnemy }) {
  ...
  return (
    <div style={{ display:"flex", gap:8, marginTop:2, flexWrap:"wrap" }}>
      <span style={{ fontSize:6, color:"#4488ff" }}>&#x25CF; {end.toLocaleString()} remain</span>
      {!isEnemy && wCapped > 0 && (
        <span style={{ fontSize:6, color:"#3daa60" }}>&#x25CF; {wCapped.toLocaleString()} healing</span>
      )}
      {killed > 0 && (
...
function CommanderPopup({ b, side, onClose }) {
  ...
  const stats     = isAtk ? b.atkCmdStats   : null;
  const tt        = troopType ? TROOP[troopType] : null;
  const cmdStats  = stats ? [
    { label:"ATK",   val:stats.atk, color:"#e08050" },
    { label:"FOCUS", val:stats.foc, color:"#50d090" },
    { label:"SPD",   val:stats.spd, color:"#d0a030" },
  ] : [];
      ...
      <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:8 }}>
        ...
        <div style={{ flex:1, textAlign:"right" }}>
          ...
          <div onClick={e => { e.stopPropagation(); setPopup(p => p==="def" ? null : "def"); }}
            style={{ fontSize:8, color:"#9a5050", fontFamily:"'Cinzel',serif", marginBottom:2, cursor:"pointer" }}>
