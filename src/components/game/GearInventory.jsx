...
function GearDetail({ piece, inventory, cmds, setCmds, setInventory, playerAlignment }) {
  ...
  const handleEquip = (cmdUid) => {
    ...
    setCmds(prev => prev.map(c => {
      if (c.uid !== cmdUid) return c;
      return { ...c, gear: { ...(c.gear ?? {}), [piece.slot]: piece.instanceId } };
    }));
    setEquipTarget(null);
  };
...
export default function GearInventory({ inventory, setInventory, cmds, setCmds, playerAlignment, filterSlot, onClose }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      {/* Filter bar */}
      <div style={{
        padding: "8px 12px", borderBottom: "1px solid #1a1508",
        background: "rgba(0,0,0,.2)", display: "flex", gap: 5, flexWrap: "wrap",
        alignItems: "center", flexShrink: 0,
      }}>
        {onClose && (
          <button onClick={onClose} style={{
            background: "none", border: "1px solid #2a2010", color: "#6a5040",
            fontFamily: "'Cinzel',serif", fontSize: 9, padding: "2px 8px",
        ...
        {Object.entries(GEAR_SLOTS).map(([key, s]) => (
          <button key={key} onClick={() => setFilterS(filterS === key ? null : key)} style={{
            ...
            background: filterS === key ? "rgba(200,160,64,.15)" : "rgba(255,255,255,.03)",
            border: `1px solid ${filterS === key ? "rgba(200,160,64,.4)" : "#1e1810"}`,
            color: filterS === key ? "#c8a040" : "#4a3a28",
            fontFamily: "'Cinzel',serif", fontSize: 7, cursor: "pointer", whiteSpace: "nowrap",
          }}>{s.icon} {s.n}</button>
        ))}
        {/* Rarity filters */}
        {Object.entries(GEAR_RARITY).reverse().map(([key, r]) => (
          <button key={key} onClick={() => setFilterR(filterR === key ? null : key)} style={{
            ...
            border: `1px solid ${filterR === key ? r.color + "50" : "#1e1810"}`,
            color: filterR === key ? r.color : "#4a3a28",
            fontFamily: "'Cinzel',serif", fontSize: 7, cursor: "pointer", whiteSpace: "nowrap",
          }}>{r.n}</button>
        ))}
        <span style={{ marginLeft: "auto", fontSize: 7, color: "#3a2e18",
