    ...
    <div style={{
      ...
      <div style={{ flex:1, overflow:"hidden", position:"relative" }}>
        <svg viewBox={`0 0 ${DW} ${DH}`} preserveAspectRatio="none"
          ...
          <polygon points="0,590 30,558 60,572 100,545 140,560 185,535 230,550 275,530 320,545 365,528 410,542 455,528 500,542 545,530 590,548 635,535 670,552 700,540 700,660 0,660"
            fill="url(#mtnGrad)" opacity={0.9} style={{pointerEvents:"none"}}/>
          {/* Snow caps on far peaks */}
          {[[30,558],[100,545],[185,535],[275,530],[365,528],[455,528],[545,530],[635,535]].map(([px,py],i) => (
            <polygon key={i} points={`${px},${py} ${px-10},${py+14} ${px+10},${py+14}`}
              fill="#d8d4cc" opacity={0.5} style={{pointerEvents:"none"}}/>
          ))}
          {/* Mid closer peaks */}
          <polygon points="0,615 25,595 55,608 90,588 130,602 175,582 220,596 270,578 320,592 370,576 420,590 470,578 520,592 565,580 610,595 650,582 685,596 700,600 700,660 0,660"
            fill="url(#mtnGrad2)" opacity={0.95} style={{pointerEvents:"none"}}/>
          {/* Fog overlay */}
          <rect x={0} y={555} width={DW} height={105} fill="url(#southFog)" style={{pointerEvents:"none"}}/>
