    ...
    <div style={{
      ...
      <div style={{ flex:1, overflow:"hidden", position:"relative" }}>
        <svg viewBox={`0 0 ${DW} ${DH}`} preserveAspectRatio="none"
          ...
          <polygon points="0,615 30,583 60,597 100,570 140,585 185,560 230,575 275,555 320,570 365,553 410,567 455,553 500,567 545,555 590,573 635,560 670,577 700,565 700,660 0,660"
            fill="url(#mtnGrad)" opacity={0.9} style={{pointerEvents:"none"}}/>
          {/* Snow caps on far peaks */}
          {[[30,583],[100,570],[185,560],[275,555],[365,553],[455,553],[545,555],[635,560]].map(([px,py],i) => (
            <polygon key={i} points={`${px},${py} ${px-10},${py+14} ${px+10},${py+14}`}
              fill="#d8d4cc" opacity={0.5} style={{pointerEvents:"none"}}/>
          ))}
          {/* Mid closer peaks */}
          <polygon points="0,635 25,615 55,628 90,608 130,622 175,602 220,616 270,598 320,612 370,596 420,610 470,598 520,612 565,600 610,615 650,602 685,616 700,620 700,660 0,660"
            fill="url(#mtnGrad2)" opacity={0.95} style={{pointerEvents:"none"}}/>
          {/* Fog overlay */}
          <rect x={0} y={575} width={DW} height={85} fill="url(#southFog)" style={{pointerEvents:"none"}}/>
