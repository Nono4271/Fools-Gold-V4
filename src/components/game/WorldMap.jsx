...
const POLYS = {
  ...
  emberpeak:         [[0,192],[205,192],[188,381],[0,381]],
  // shallows split into 3 parts — rendered together, same key
  shatteredShallows: [[170,120],[310,120],[310,280],[197,280],[205,192],[192,192]],
  _shallows_C:       [[310,120],[390,120],[390,240],[310,240]],
  _shallows_R:       [[390,120],[530,120],[500,220],[503,280],[390,280],[390,240]],
  ironhaven:         [[495,192],[700,192],[700,381],[512,381]],
  cinderplain:       [[0,381],[175,381],[175,496],[0,496]],
  stormwatch:        [[525,381],[700,381],[700,496],[525,496]],
  ashenRift:         [[175,280],[310,280],[310,479],[350,479],[350,580],[175,580]],
  holyGrail:         [[310,240],[390,240],[390,479],[310,479]],
  bloodmarch:        [[390,280],[525,280],[525,580],[350,580],[350,479],[390,479],[390,280]],
  runemarks:         [[175,580],[350,580],[350,700],[200,700],[175,630]],
  boneridge:         [[350,580],[525,580],[525,630],[500,700],[350,700]],
  ashenveil:         [[0,496],[175,496],[175,630],[200,700],[0,700]],
  grimhold:          [[525,496],[700,496],[700,700],[500,700],[525,630]],
};

// Internal keys that share the shallows region visually
const SHALLOWS_PARTS = new Set(['shatteredShallows', '_shallows_C', '_shallows_R']);
const INTERNAL_KEYS  = new Set(['_shallows_C', '_shallows_R']);
...
export default function WorldMap({ tiles, onClose, onTeleport, panRef, zoom }) {
  ...
  // Resolve display key: _shallows_* parts display as shatteredShallows
  const displayKey = (key) => SHALLOWS_PARTS.has(key) ? 'shatteredShallows' : key;
    ...
    <div style={{
      ...
      <div style={{ flex: 1, overflow: "hidden", position: "relative" }}>
        <svg viewBox={`0 0 ${DW} ${DH}`} preserveAspectRatio="none"
          ...
          {/* ── Region polygons ── */}
          {/* Render all POLYS entries including internal shallows parts */}
          {Object.entries(POLYS).map(([rawKey, poly]) => {
            const regKey = displayKey(rawKey);
            const reg = keeps.find(k => k.key === regKey);
            if (!reg) return null;

            const isSel  = selected === regKey;
            const owned  = reg.owner;
            const facCol = owned ? (owned === "player" ? "#44aaff" : (FAC_COLOR[owned] || "#cc8844")) : null;
            const isHG   = regKey === "holyGrail";

            return (
              <g key={rawKey} style={{ cursor: "pointer" }}
                onClick={() => !INTERNAL_KEYS.has(rawKey) && setSelected(isSel ? null : regKey)}>
          ...
          {/* ── Region labels (one per logical region, skip internal parts) ── */}
          {keeps.map(reg => {
            if (reg.key === "holyGrail") return null;
            if (INTERNAL_KEYS.has(reg.key)) return null;

            // For shallows use centroid of the combined bounding area
            let poly = POLYS[reg.key];
            if (reg.key === "shatteredShallows") {
              // use centre of top strip as label position
              poly = [[170,120],[530,120],[503,280],[197,280]];
            }
            if (!poly) return null;
