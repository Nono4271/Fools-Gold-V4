...
import { ISO_W, ISO_H, TW, TH, ROWS, TOP_PAD } from "../../constants/geometry.js";
...
export default function WorldMap({ tiles, onClose, onTeleport, panSt, zoom }) {
  ...
  // Letterbox: maintain DW:DH aspect ratio, no stretch
  ...
  // Center the map horizontally if narrower than screen
  const offsetX = Math.round((screenW - mapW) / 2);
      ...
      <div style={{ flex:1, overflow:"hidden", position:"relative", display:"flex", justifyContent:"center" }}>
        <svg width={mapW} height={mapH} style={{ display:"block" }}>
          ...
          {keeps.map(reg => {
            ...
            if (!poly) return null;
            const terrain = REGION_TERRAIN[reg.key] || { base:"#1e2818", dark:"#141e10" };
            ...
            return (
              <g key={reg.key} style={{cursor:"pointer"}}
                ...
                <polygon points={scalePts(poly,sx,sy)} fill={terrain.base}/>
                <polygon points={scalePts(poly,sx,sy)} fill={terrain.dark} opacity={0.4}/>
                {facCol && <polygon points={scalePts(poly,sx,sy)} fill={facCol} opacity={0.28}/>}
                {!owned && !isHG && <polygon points={scalePts(poly,sx,sy)} fill="#000" opacity={0.38}/>}
                {isSel  && <polygon points={scalePts(poly,sx,sy)} fill="white" opacity={0.07}/>}
                <polygon points={scalePts(poly,sx,sy)} fill="none"
                  stroke={facCol || (isHG ? "#f0c040" : "#4a3a20")}
                  strokeWidth={isSel ? 2 : 0.8}
                  strokeOpacity={facCol ? 0.7 : (isHG ? 0.6 : 0.32)}
          ...
          {/* ── Player viewport dot ── */}
          {panSt && zoom && (() => {
            // World-space pixel at screen centre (inverse of isoXY offsets)
            const worldX = (screenW / 2 - panSt.x) / zoom - (ROWS * TW / 2);
            const worldY = (screenH / 2 - panSt.y) / zoom - TOP_PAD;
            // Recover iso col/row, then map to SVG design space
            const isoC = (worldX / (TW / 2) + worldY / (TH / 2)) / 2;
            const isoR = (worldY / (TH / 2) - worldX / (TW / 2)) / 2;
            const dotX = (isoC / 700) * DW * sx;
            const dotY = (isoR / 700) * DH * sy;
          ...
          {keeps.map(reg => {
            ...
            const isHG  = reg.key === "holyGrail";
            const sz    = isHG ? 9*sx : reg.layer==="conflict" ? 7*sx : 6*sx;
            const isSel = selected === reg.key;
            const by    = cy - sz*1.2;
          ...
          {/* ── Legend ── */}
          <rect x={0} y={mapH-34} width={mapW} height={34} fill="rgba(0,0,0,0.75)"/>
          {[
