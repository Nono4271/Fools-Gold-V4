  ...
  return (
    <div style={{
      ...
      <svg width="96" height="96" viewBox={`0 0 ${COLS} ${ROWS}`} style={{display:"block"}}>
        ...
        {/* Viewport rect — approximate in grid space using isometric inverse */}
        <rect
          x={Math.max(0, (-panSt.x / TW + -panSt.y / (TH / 2)) / 2 - COLS * 0.07)}
          y={Math.max(0, (-panSt.y / (TH / 2) - -panSt.x / TW) / 2 - ROWS * 0.07)}
          width={Math.min(COLS * 0.25, (window.innerWidth / TW + (window.innerHeight - 52) / (TH / 2)) / 2)}
          height={Math.min(ROWS * 0.25, (window.innerWidth / TW + (window.innerHeight - 52) / (TH / 2)) / 2)}
