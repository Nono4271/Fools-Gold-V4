...
const tickAiRss = useCallback(() => {
  ...
  setAiRss(p => {
    ...
    // Fix: Object.values() on a 490k-tile map built a massive array every second.
    // Object.keys() iterates the same data without allocating the values array.
...
const tickAiEcon = useCallback(() => {
  ...
  // Fix: was calling Object.values(tilesRef.current).some(...) for every AI commander
  // — O(commanders × 490,000 tiles). Now does a direct key lookup O(1) per commander.
  const aiHQTileKeys = new Set(
    Object.keys(tilesRef.current).filter(k => {
      const t = tilesRef.current[k];
      return t.isHQ && t.owner === "ai";
    })
  );
