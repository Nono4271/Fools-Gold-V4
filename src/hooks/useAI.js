...
// Fix: use rssRate() to match the player's exponential resource formula
...
}, [screen, aiFaction, setAiRss, tilesRef, aiBldgsRef]);

// AI march tick (throttled to 45s)
useEffect(() => {
if (screen !== "game" || !aiFaction) return;
const id = setInterval(() => {
const now = Date.now();
...
}, [screen, aiFaction, cmdsRef, tilesRef, aiLastActionRef, setCmds]);

// AI economy tick (every 5s — train, assign, upgrade)
useEffect(() => {
if (screen !== "game" || !aiFaction) return;
const id = setInterval(() => {
const curCmds    = cmdsRef.current;
...
const aiCmds     = curCmds.filter(c => c.owner === "ai");
  ...
  // Strategy: fill main-branch skills first (strongest single investment), then side skills.
  ...
  if (cmdsWithPoints.length) {
    ...
    // Try main skills of each branch first, then side skills
  ...
  // HQ gates everything via maxAvailLevel, so without it AI is permanently stuck at lvl1.
...
}, [screen, aiFaction, cmdsRef, aiRssRef, aiBldgsRef, aiPoolRef, setCmds, setAiRss, setAiBldgs, setAiBarracksPool]);
}
