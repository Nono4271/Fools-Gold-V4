...
import { POWER_DEFS, HQP, AI_HQ_KEY, WIN_KEY, SIEGE_BASE, KEEP_GARRISON_RESET_MS } from "../constants/map.js";
...
arrivedAttackers.forEach(cmd => {
  ...
  if (defTile.garrisonDefeated) {
    ...
    if (siegePower >= currentSiege) {
      ...
      setTiles(p => ({ ...p, [destKey]:{ ...p[destKey], siege:currentSiege-siegePower, resetAt:Date.now()+garrisonResetMs(defTile) } }));
  ...
  if (hasAiCmd) {
    ...
    if (!res2.won) {
      ...
      setTiles(p => ({ ...p, [destKey]:{ ...p[destKey], defCmd:null, hasAiCommander:false, garrisonDefeated:true, resetAt:Date.now()+garrisonResetMs(defTile) } }));
      setCmds(p => p.map(c => {
        if (c.uid !== cmd.uid) return c;
        const retreatPath = bfsPath(originKey, hqKey);
        const stepMs = marchStepMs(effectiveMarchSpd(boostedCmd.spd||60, null, boostedCmd.gearBonuses?.armySpd || 0));
  ...
  if (siegePower >= currentSiege) {
    ...
    setTiles(p => ({ ...p, [destKey]:{ ...p[destKey], siege:currentSiege-siegePower, garrisonDefeated:true, resetAt:Date.now()+garrisonResetMs(defTile), defCmd:null, hasAiCommander:false } }));
    ...
    drawCmds.forEach(cmd => {
      ...
      if (siegePower >= currentSiege) {
        ...
        setTiles(p => ({ ...p, [destKey]:{ ...p[destKey], siege:currentSiege-siegePower,
          garrisonDefeated:true, resetAt:Date.now()+garrisonResetMs(defTile),
          defCmd:null, hasAiCommander:false } }));
        floaty(`⚔ SIEGE ${currentSiege-siegePower}/${defTile.siegeMax??SIEGE_BASE}`, "#d0a030", destKey);
      }
...
arrivedAI.forEach(cmd => {
  ...
  if (!hasFoothold) {
    setCmds(p => p.map(c => c.uid === cmd.uid ? { ...c, march:null, tk:getAiHqKey(cmd) } : c));
    return;
  }
  ...
  if (res.won) {
    ...
    if (siegePower >= currentSiege) {
      ...
      setTiles(p => ({ ...p, [destKey]:{ ...p[destKey], siege:currentSiege-siegePower, garrisonDefeated:true, resetAt:Date.now()+garrisonResetMs(defTile) } }));
    }
  }
