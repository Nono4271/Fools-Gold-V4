...
// If still at drawTile and adjacency is valid → re-trigger battle.
// If they moved away or adjacency lost → clear timer silently.
useEffect(() => {
  if (screen !== "game") return;
  const id = setInterval(() => {
    const now = Date.now();
    ...
    drawCmds.forEach(cmd => {
      ...
      // Timer expired — trigger rematch
      floaty("⚔ REMATCH!", "#c0a020", destKey);
      setCmds(p => p.map(c => c.uid === cmd.uid
        ? { ...c, drawTimer:null, drawTile:null, drawOrigin:null,
            march:{ type:"attack", path:[originKey, destKey], step:0,
                    dest:destKey, origin:originKey,
                    stepMs:1, lastStepTime:0, arrived:false } }
        : c));
    });
  }, 1000);
  return () => clearInterval(id);
}, [screen, cmds, tilesRef, setCmds, floaty, hqKey]);
