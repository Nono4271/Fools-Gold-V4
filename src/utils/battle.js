...
export function simBattle(cmd, attackerTroops, defTile, wallLvl) {
  ...
  const CMD_ATK_SCALE = 62;
  const atkCmdAtk = (cmd.atk || 150) * CMD_ATK_SCALE * Math.pow(1.20, atkLvl - 5) * passives.cmdAtkMult;
  ...
  const defCmdAtk = (dc ? dc.atk : 80) * CMD_ATK_SCALE * Math.pow(1.20, Math.max(0, defLvl - 2));
  ...
  const troopDmg = (aTT, dTT, atkCount, defHP, lvlMult, terrMult, isAtk, resistMult, atkMult) => {
    ...
    if (aTT.dmgType === "magical") {
      raw    = Math.max(1, Math.ceil(atkCount)) * aTT.focus * lvlMult * terrMult * (0.85+Math.random()*0.30) * (isAtk?armyFocMult:1) * (atkMult||1) / 68;
      resist = calcMagRes(dTT) * (resistMult||1);
    } else {
      raw    = Math.max(1, Math.ceil(atkCount)) * aTT.atk   * lvlMult * terrMult * (0.85+Math.random()*0.30) * (isAtk?armyAtkMult:1) * (atkMult||1) / 68;
