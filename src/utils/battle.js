...
// ── Gear army boosts (applied as flat % multipliers to troop damage) ──
const gb          = cmd.gearBonuses || {};
const armyAtkMult = 1 + (gb.armyAtk   || 0) / 100;
const armyFocMult = 1 + (gb.armyFoc   || 0) / 100;
const armySpdBonus=      gb.armySpd   || 0;   // added to troop speed for initiative
const armySiege   =      gb.armySiege || 0;   // added to siege power (useMarch handles)
...
// Defender Lv25 Bastion: attacker troops get double HP and double DEF for rounds 1-2
...
const applyTroopDmg = (atkTroopType, defTroopType, atkCount, defHP, lvlMult, terrMult = 1, isAttacker = false, defResistMult = 1) => {
const aTT = TROOP[atkTroopType];
const dTT = TROOP[defTroopType] || null;
if (!aTT) return { dmg:0, newHp:defHP };
let raw, resist;
if (aTT.dmgType === "magical") {
raw    = calcMagAtk(aTT, atkCount, lvlMult, terrMult) * (isAttacker ? armyFocMult : 1);
resist = calcMagRes(dTT) * defResistMult;
} else {
raw    = calcPhysAtk(aTT, atkCount, lvlMult, terrMult) * (isAttacker ? armyAtkMult : 1);
resist = calcPhysDef(dTT) * defResistMult;
}
...
// Defender Bastion: double DEF stat for attacker troops in rounds 1–2
...
if (bastionActive && round === 1) {
  roundLog.actions.push({ actor:cmd.n, action:"🛡 BASTION — double HP & DEF active (rounds 1-2)", dmg:0, isSkill:true });
}
...
const res = applyTroopDmg(cmd.troopType, dc?.troopType || defTile.troopType, atkCount * thisTroopMult, defTroopHp, atkLvlMult, 1, true);
...
const atkRes = (atkTT ? calcPhysDef(atkTT) : 30) * bastionDefMult;
...
const res = applyTroopDmg(dc?.troopType || defTile.troopType, cmd.troopType, defCount * defDmgMult, atkTroopHp, defLvlMult, defTerrBonus, false, bastionDefMult);
...
// Divide by bastionHpMult so doubled HP doesn't undercount losses
const finalAtkLost = won
? Math.min(attackerTroops - 1, Math.max(1, Math.round((atkTroopHpStart - Math.max(0, atkTroopHp)) / (atkTroopHpPer * bastionHpMult))))
