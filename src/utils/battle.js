...
export function simBattle(cmd, attackerTroops, defTile, wallLvl) {
  ...
  const atkCmdAtk = (cmd.atk || 150) * Math.pow(1.20, atkLvl - 5) * passives.cmdAtkMult;
  ...
  const troopDmg = (aTT, dTT, atkCount, defHP, lvlMult, terrMult, isAtk, resistMult, atkMult) => {
    ...
    if (aTT.dmgType === "magical") {
      raw    = Math.max(1, Math.ceil(atkCount)) * aTT.focus * lvlMult * terrMult * (0.85+Math.random()*0.30) * (isAtk?armyFocMult:1) * (atkMult||1) / 10;
      resist = calcMagRes(dTT) * (resistMult||1);
    } else {
      raw    = Math.max(1, Math.ceil(atkCount)) * aTT.atk   * lvlMult * terrMult * (0.85+Math.random()*0.30) * (isAtk?armyAtkMult:1) * (atkMult||1) / 10;
  ...
  if (passives.cmdAtkMult > 1)
    phase0.actions.push({ actor:cmd.n, action:`Passive: +${Math.round((passives.cmdAtkMult-1)*100)}% Commander ATK`, dmg:0, isSkill:true, isPhase0:true });
  if (passives.critChance > 0)
    phase0.actions.push({ actor:cmd.n, action:`Passive: +${Math.round(passives.critChance*100)}% Critical Hit Chance`, dmg:0, isSkill:true, isPhase0:true });
  if (passives.dmgReduce > 0)
    phase0.actions.push({ actor:cmd.n, action:`Passive: -${Math.round(passives.dmgReduce*100)}% Incoming Damage`, dmg:0, isSkill:true, isPhase0:true });
  if (passives.enemyAtkReduce > 0)
    phase0.actions.push({ actor:cmd.n, action:`Passive: -${Math.round(passives.enemyAtkReduce*100)}% Enemy ATK`, dmg:0, isSkill:true, isPhase0:true });
  if (passives.troopAtkMult > 1)
    phase0.actions.push({ actor:cmd.n, action:`Passive: +${Math.round((passives.troopAtkMult-1)*100)}% Troop ATK`, dmg:0, isSkill:true, isPhase0:true });
  if (passives.troopDefMult > 1)
    phase0.actions.push({ actor:cmd.n, action:`Passive: +${Math.round((passives.troopDefMult-1)*100)}% Troop DEF`, dmg:0, isSkill:true, isPhase0:true });
  if (passives.healPerRound > 0)
    phase0.actions.push({ actor:cmd.n, action:`Passive: +${Math.round(passives.healPerRound*100)}% Troops Restored per Round`, dmg:0, isSkill:true, isPhase0:true });
  if (passives.garrisonIgnore > 0)
    phase0.actions.push({ actor:cmd.n, action:`Passive: Ignore ${Math.round(passives.garrisonIgnore*100)}% Garrison Bonus`, dmg:0, isSkill:true, isPhase0:true });
  ...
  for (let round = 1; round <= 10; round++) {
    ...
    // Deduplicate skill names logged by both duration+instant
    const uniqNames = [...new Set(rs.skillFiredNames)];
    uniqNames.forEach(name => roundLog.actions.push({ actor:cmd.n, action:`✨ ${name}`, dmg:0, isSkill:true }));
    ...
    if (rs.healPct > 0 && totalAtkLostHp > 0 && !healBlocked) {
      ...
      if (troopsBack > 0)
        roundLog.actions.push({ actor:cmd.n, action:`💚 ${troopsBack} troops restored`, dmg:-troopsBack, isSkill:true, isHeal:true });
    }
    ...
    for (const ent of order) {
      ...
      if (ent.id === "atkCmd") {
        ...
        for (let h = 0; h < rs.cmdHits; h++) {
          ...
          defTroopHp = Math.max(0, defTroopHp - dmg);
          roundLog.actions.push({ actor:cmd.n, action:`${cmd.n} struck enemy troops${isCrit?" (CRIT!)":""}`, dmg, isPlayer:true });
        }
        ...
        roundLog.actions.push({ actor:"Troops", action:`${atkTT.icon||"⚔"} ${atkTT.label||"Troops"} attacked`, dmg:res.dmg, isPlayer:true });
        ...
        const dmg    = Math.max(1, Math.round(raw * red));
        const prev   = atkTroopHp;
        atkTroopHp   = Math.max(0, atkTroopHp - dmg);
        totalAtkLostHp += (prev - atkTroopHp);
        ...
        atkTroopHp   = Math.max(0, atkTroopHp - dmg);
        totalAtkLostHp += (prev - atkTroopHp);
        roundLog.actions.push({ actor:"Defenders", action:`${defTT.icon} ${defTT.label} attacked your soldiers`, dmg, isPlayer:false });
      }
    }
  ...
  const finalAtkLost  = won
    ? Math.min(attackerTroops - 1, Math.max(1, Math.round((atkHpMax - Math.max(0, atkTroopHp)) / (atkTroopHpPer * bastionHpMult))))
