...
function drawAllTiles(gfx, tiles, rMin, rMax, cMin, cMax, selKey, mode, cByTile, mvCmdUid, zoom = 1) {
  ...
  for (let d = dMin; d <= dMax; d++) {
    ...
    for (let c = cLo; c <= cHi; c++) {
      ...
      // Draw shore tiles as ocean — fills gaps at map edge cleanly
      if (isShore) {
        const { cx, cy } = isoXY(c, r);
        const mid = cy + TH / 2;
...
function drawAllProps(gfx, tiles, rMin, rMax, cMin, cMax) {
  ...
  for (let d = dMin; d <= dMax; d++) {
    ...
    for (let c = cLo; c <= cHi; c++) {
      ...
      if (r < rMin || r > rMax) continue;
      const tile = tiles[`${c},${r}`];
      if (!tile || tile.isHQ || tile.isWin || tile.isKeep || tile.isKeepPart || tile.isHQPart) continue;
      const { cx, cy } = isoXY(c, r);
      const sy = cy - 4;
      if (tile.rss) {
...
══════════════════════════════════════════════════════════════════════════ */
export const MapRenderer = forwardRef(function MapRenderer({ tiles, cmds, reinMarches, selKey, mode, mvCmd, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }, ref) {
  ...
  useEffect(() => {
    ...
    let app;
    try {
      app = new PIXI.Application({ width:w, height:h, backgroundColor:0x1a3a5c,
        antialias:false, resolution:Math.min(window.devicePixelRatio||1,2), autoDensity:true });
    } catch(_) {
      try { app = new PIXI.Application({ width:w, height:h, backgroundColor:0x1a3a5c, forceCanvas:true }); }
  ...
  /* ── Imperative teleport — bypasses React state, zero re-render lag ── */
  useImperativeHandle(ref, () => ({
    teleport(x, y) {
      panRef.current = { x, y };
      if (worldRef.current) { worldRef.current.x = x; worldRef.current.y = y; }
      lastBoundsRef.current = null;
      redrawRef.current?.redraw(true);
    }
  }), []);
