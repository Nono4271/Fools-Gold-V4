...
function drawGapFills(gfx, tiles, rMin, rMax, cMin, cMax) {
  ...
  const tileElev = t => t.isHQ ? 14 : t.isWin ? 10 : t.isKeep ? 8 : 4;

  // ── Horizontal seams: between (c,r) and (c+1,r) ──────────────────────────
  for (let r = rMin; r <= rMax; r++) {
    for (let c = cMin; c < Math.min(cMax, COLS - 1); c++) {
      const t1 = tiles[`${c},${r}`], t2 = tiles[`${c+1},${r}`];
      if (!t1 || !t2) continue;
      ...
      const xS = cx1 + TW / 2;
      const ey = ((cy1 - tileElev(t1)) + (cy2 - tileElev(t2))) / 2;
  ...
  // ── Diagonal seams: between (c,r) and its two lower neighbours ───────────
  // Even rows are offset by TW/2; odd rows by TW. Fill the triangular gap
  // at each shared vertex so no dark hatch lines appear at low zoom.
  for (let r = rMin; r < Math.min(rMax, ROWS - 1); r++) {
    const isEven = r % 2 === 0;
    for (let c = cMin; c <= cMax; c++) {
      const tT = tiles[`${c},${r}`];
      if (!tT) continue;
      const vT = TV[tT.terrain] || TV_DEF;
      const { cx: cxT, cy: cyT } = isoXY(c, r);
      const syT = cyT - tileElev(tT);

      // Lower-left neighbour
      const cLL = isEven ? c - 1 : c;
      const tLL = tiles[`${cLL},${r+1}`];
      if (tLL) {
        const vLL = TV[tLL.terrain] || TV_DEF;
        const { cx: cxLL, cy: cyLL } = isoXY(cLL, r+1);
        const syLL = cyLL - tileElev(tLL);
        const x0 = cxT - TW / 2,  y0 = syT  + TH / 2;
        const x1 = cxT,            y1 = syT  + TH;
        const x2 = cxLL + TW / 2,  y2 = syLL + TH / 2;
        const mx = (x0+x1+x2)/3,   my = (y0+y1+y2)/3;
        gfx.beginFill(vT.acc,  0.7); gfx.drawPolygon([x0,y0, x1,y1, mx,my]); gfx.endFill();
        gfx.beginFill(vLL.acc, 0.7); gfx.drawPolygon([x1,y1, x2,y2, mx,my]); gfx.endFill();
      }

      // Lower-right neighbour
      const cLR = isEven ? c : c + 1;
      const tLR = tiles[`${cLR},${r+1}`];
      if (tLR) {
        const vLR = TV[tLR.terrain] || TV_DEF;
        const { cx: cxLR, cy: cyLR } = isoXY(cLR, r+1);
        const syLR = cyLR - tileElev(tLR);
        const x0 = cxT,            y0 = syT  + TH;
        const x1 = cxT + TW / 2,   y1 = syT  + TH / 2;
        const x2 = cxLR - TW / 2,  y2 = syLR + TH / 2;
        const mx = (x0+x1+x2)/3,   my = (y0+y1+y2)/3;
        gfx.beginFill(vT.acc,  0.7); gfx.drawPolygon([x0,y0, x1,y1, mx,my]); gfx.endFill();
        gfx.beginFill(vLR.acc, 0.7); gfx.drawPolygon([x0,y0, x2,y2, mx,my]); gfx.endFill();
      }
    }
  }
}
...
export function MapRenderer({ tiles, cmds, reinMarches, selKey, mode, mvCmd, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }) {
  ...
  useEffect(() => {
    ...
    app.ticker.add(() => {
      ...
      // Extra column buffer accounts for even/odd row TW/2 stagger.
      ...
      const cMin = Math.max(0,        Math.floor(wxL / TW) - SPAWN_BUF - 2);
      const cMax = Math.min(COLS - 1, Math.ceil( wxR / TW) + SPAWN_BUF + 2);
