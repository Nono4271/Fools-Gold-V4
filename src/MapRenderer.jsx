...
export const MapRenderer = memo(forwardRef(function MapRenderer({ tiles, cmds, selKey, mode, mvCmd, reinMarchesRef, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }, ref) {
  ...
  useEffect(() => {
    ...
    function redraw(force = false) {
      ...
      console.log('[redraw]', { force, pan: {...panRef.current}, worldX: worldRef.current?.x, worldY: worldRef.current?.y, isPanning: isPanning.current });
      ...
      // Swap: make back visible, hide front. Do NOT clear front yet —
      // clear it on the next ticker frame after the GPU has rendered the new back.
      back.visible  = true;
      front.visible = false;
      tileBackRef.current  = front;
      tileFrontRef.current = back;
      // Schedule the clear for next frame so there's never a frame with nothing drawn
      requestAnimationFrame(() => { front.clear(); });
      ...
      if (z >= 1.0) {
        ...
        requestAnimationFrame(() => { pf.clear(); });
      } else {
      ...
      if (e.touches.length === 0) {
        if (!tDidDrag.current) {
          ...
          if (key) {
            selRef.current = key;
            drawSelection(key);
            console.log('[onTE] calling onTileClick', key, Date.now());
            onTileClickRef.current(key, e);
            console.log('[onTE] onTileClick returned', Date.now());
          }
        }
        console.log('[onTE] pan end — isPanning→false, lastBounds→null');
  ...
  useEffect(() => {
    console.log('[panSt effect] firing', panSt, 'worldX before:', worldRef.current?.x);
  ...
  useEffect(() => {
    console.log('[selKey/mode effect] firing', { selKey, mode, isPanning: isPanning.current });
    lastBoundsRef.current = null;
