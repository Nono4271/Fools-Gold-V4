...
function drawKeepGfx(gfx, bx, by, owner, isWin, isSelected) {
  ...
  if (isSelected) {
    ...
    gfx.drawEllipse(bx, by - 10, 44, 18);
    gfx.lineStyle(0);
  } else if (owner) {
    gfx.lineStyle(1.5, fc, 0.5);
    gfx.drawEllipse(bx, by - 10, 44, 18);
    gfx.lineStyle(0);
  }
}
...
function buildKeepLayer(keepCont, tiles, selKey, onKeepClick, PIXI) {
  ...
  for (const reg of KEEP_REGION_LIST) {
    ...
    const elev = 8;
    const by   = worldCY - elev - 10; // raise keep landmark above tile surface

    const isSelected = selKey === tileKey;
    const owner      = tile.owner || null;

    // Graphics for the keep visual
    const gfx = new PIXI.Graphics();
    drawKeepGfx(gfx, bx, by, owner, reg.isWin, isSelected);
    keepCont.addChild(gfx);

    // Invisible hit area — large enough for comfortable tapping
    const hit = new PIXI.Graphics();
    hit.beginFill(0xffffff, 0.001);
    hit.drawEllipse(bx, by - 10, 44, 28);
    hit.endFill();
