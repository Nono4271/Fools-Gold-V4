...
export function MapRenderer({ tiles, cmds, reinMarches, selKey, mode, mvCmd, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }) {
  ...
  useEffect(() => {
    ...
    app.ticker.add(() => {
      if (isPanning.current) needsFullRedraw.current = true;
      ...
      /* Safety cap: never iterate more than ~3000 tiles per frame */
      const rowCount = rMax - rMin + 1;
      const colCount = cMax - cMin + 1;
      if (rowCount * colCount > 3000) return;
      ...
      // Remove out-of-viewport tiles (use larger buffer to avoid pop-in during fast pan)
      const EVICT_BUF = 6;
      for (const [k, entry] of curGfxMap) {
        const comma = k.indexOf(",");
        const c = +k.slice(0, comma), r = +k.slice(comma + 1);
        if (r < rMin - EVICT_BUF || r > rMax + EVICT_BUF || c < cMin - EVICT_BUF || c > cMax + EVICT_BUF) {
