...
export function MapRenderer({ tiles, cmds, reinMarches, selKey, mode, mvCmd, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }) {
    ...
    app.ticker.add(() => {
      ...
      for (let r = rMin; r <= rMax; r++) {
        for (let c = cMin; c <= cMax; c++) {
          ...
          const hash    = tileHash(tile, selK, modV, hasCmds, mvUid);
          ...
          if (!entry) {
            ...
            drawTile(gfx, tile, selK, modV, hasCmds, mvUid);
          } else if (entry.hash !== hash) {
            entry.hash = hash;
            drawTile(entry.gfx, tile, selK, modV, hasCmds, mvUid);
          }
        }
      }
  ...
  useEffect(() => {
    ...
    for (const [key, entry] of gfxMap) {
      ...
      if (!tile) continue;
      const hasCmds = Boolean(cByTile[key]?.length);
      const hash    = tileHash(tile, selKey, mode, hasCmds, mvCmd?.uid);
      if (entry.hash !== hash) {
        entry.hash = hash;
