...
export const MapRenderer = memo(forwardRef(function MapRenderer({ tiles, cmds, selKey, mode, mvCmd, reinMarchesRef, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }, ref) {
    ...
    const onTE = e => {
      ...
      if (e.touches.length === 0) {
        if (!tDidDrag.current) {
          ...
          if (key) {
            selGfx.clear(); // erase previous outline instantly on canvas
            selRef.current = key;
            drawSelection(key);
            onTileClickRef.current(key, e);
          }
        }
        // Keep isPanning=true through the rAF so the ticker can't fire redraw()
        // between the last touchmove and when the canvas has settled.
        // This is what causes the snap on quick swipes on iOS.
        const snapPan = { x: world.x, y: world.y }; // capture exact canvas position
        panRef.current = snapPan;
        requestAnimationFrame(() => {
          isPanning.current = false;
          tDidDrag.current = false;
          lastBoundsRef.current = null;
        });
        onPanChangeRef.current(snapPan);
      }
    };
  ...
  useEffect(() => {
    // Only clear/redraw the selection outline — redrawOverlays is expensive
    // and not needed just because the selected tile changed.
