...
export const MapRenderer = memo(forwardRef(function MapRenderer({ tiles, cmds, selKey, mode, mvCmd, reinMarchesRef, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }, ref) {
  ...
  useEffect(() => {
    ...
    function redraw(force = false) {
      // Snapshot pan before drawing — if pan changes mid-draw (quick second pan)
      // we discard the stale result instead of flashing wrong tiles.
      const panSnapshot = panRef.current;
      ...
      // If a new pan started while we were drawing, discard this frame to avoid
      // flashing tiles drawn for the wrong camera position.
      if (panRef.current !== panSnapshot) {
        lastBoundsRef.current = null; // force fresh redraw when pan settles
        return;
      }
    ...
    const onTS = e => {
      ...
      if (e.touches.length === 1) {
        ...
        // Do NOT set isPanning=true here — that would suppress the ticker redraw
        // that fills in edge tiles after the previous pan ended.  isPanning is set
        // to true only once actual movement is detected in onTM.
  ...
  useEffect(() => {
    ...
    // Defer the full redraw to the next animation frame — calling redraw(true) synchronously
    // here blocks the browser from painting the React state update (popup appearing, etc.)
    // which is the main cause of the perceived click delay.
