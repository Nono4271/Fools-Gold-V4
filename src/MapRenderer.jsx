...
export const MapRenderer = forwardRef(function MapRenderer({ tiles, cmds, reinMarches, selKey, mode, mvCmd, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }, ref) {
  ...
  useEffect(() => {
    ...
    // No ticker — all redraws are explicit:
    // touchend → redraw(true), state changes → redraw(true), teleport → redraw(true)
    ...
    const onTE = e => {
      ...
      if (e.touches.length === 0) {
        if (!tDidDrag.current) {
          ...
          if (key) {
            // Flash highlight immediately — don't wait for React's setState cycle
            selRef.current = key;
            lastBoundsRef.current = null;
            redraw(true);
