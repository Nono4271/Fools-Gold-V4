...
function buildKeepLayer(keepCont, tiles, selKey, onKeepClick, PIXI) {
  ...
  for (const reg of KEEP_REGION_LIST) {
    ...
    // ── Footprint boundary — traces the outer edges of the 11×11 tile footprint ──
    // Each tile renders elevated by elev=8. Extreme corner calculations:
    //   top:    tile(0,-5) top corner    → (bx,       worldCY - 88)
    //   right:  tile(5, 0) right corner  → (bx + 192, worldCY +  8)
    //   bottom: tile(0, 5) bottom corner → (bx,       worldCY + 104)
    //   left:   tile(-5,0) left corner   → (bx - 192, worldCY +  8)
    const gy = worldCY; // tile cy (elevation handled in offsets above)
    const FOOTPRINT = [
       bx,        gy -  88,  // top
       bx + 192,  gy +   8,  // right
       bx,        gy + 104,  // bottom
    ...
    // Covers the entire iso diamond so clicking anywhere inside opens popup.
    const hit = new PIXI.Graphics();
    hit.beginFill(0xffffff, 0.001); // nearly invisible but hittable
...
export const MapRenderer = memo(forwardRef(function MapRenderer({ tiles, cmds, selKey, mode, mvCmd, reinMarchesRef, panRef: panRefProp, zoomRef: zoomRefProp, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }, ref) {
  ...
  useEffect(() => {
    ...
    const keepCont = new PIXI.Container(); world.addChild(keepCont); keepContRef.current = keepCont;
