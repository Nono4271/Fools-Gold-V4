...
export const MapRenderer = memo(forwardRef(function MapRenderer({ tiles, cmds, selKey, mode, mvCmd, reinMarchesRef, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }, ref) {
  ...
  const tTouchEnded = useRef(false);
  ...
  useEffect(() => {
    ...
    const onTS = e => {
      ...
      if (e.touches.length === 1) {
        ...
        tTouchEnded.current = false; // finger is down — allow moves
    ...
    const onTM = e => {
      e.preventDefault();
      if (tTouchEnded.current) return; // finger already lifted — ignore late move events
      if (e.touches.length === 1) {
    ...
    const onTE = e => {
      ...
      if (e.touches.length === 0) {
        tTouchEnded.current = true; // block any late touchmove events from this swipe
        if (!tDidDrag.current) {
        ...
        tDidDrag.current = false;
        // Defer the post-pan tile fill by one rAF — this lets the browser
        // commit the final canvas position first, preventing the stutter/snap
        // that occurred when drawAllTiles blocked the finger-lift frame.
  ...
  /* ── Sync pan from props (initial spawn only) ── */
  // panSt is set exactly once — when the map gen worker finishes and places the HQ.
  // After that, all pan goes through panRef via native events. We apply panSt only
  // when worldRef exists and the current world position is still at the default {4,4},
  // meaning this is the first real spawn position and not a stale re-fire during gameplay.
  const spawnApplied = useRef(false);
  useEffect(() => {
    if (!worldRef.current) return;
    if (spawnApplied.current) return; // only apply once — never overwrite post-drag position
    if (Math.abs(panSt.x) < 10 && Math.abs(panSt.y) < 10) return; // still at default, skip
    spawnApplied.current = true;
    panRef.current = panSt;
    worldRef.current.x = panSt.x;
    worldRef.current.y = panSt.y;
    lastBoundsRef.current = null;
    redrawRef.current?.redraw(true);
  }, [panSt]);
  ...
  useEffect(() => {
    // Selection change: just redraw the outline, no full tile redraw needed
