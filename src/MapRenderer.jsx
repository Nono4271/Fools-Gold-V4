...
function buildKeepLayer(keepCont, tiles, selKey, onKeepClick, PIXI) {
  ...
  for (const reg of KEEP_REGION_LIST) {
    ...
    // ── Footprint boundary in iso world space ──────────────────────────────
    // Convex hull of the 11×11 tile footprint (R=5) relative to keep center.
    // Computed from tile corners: a 4-point diamond.
    // Offsets: top=(0,-160), right=(+352,+16), bottom=(0,+192), left=(-352,+16)
    // "by" here is the elevated graphic origin; the footprint sits at worldCY (ground).
    const gy = worldCY + TH / 2; // ground center y of keep tile
    const FOOTPRINT = [
       bx,        gy - 160,  // top
       bx + 352,  gy + 16,   // right
       bx,        gy + 192,  // bottom
