...
export const MapRenderer = forwardRef(function MapRenderer({ tiles, cmds, reinMarches, selKey, mode, mvCmd, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }, ref) {
  ...
  useEffect(() => {
    ...
    /* ── Ticker: only used to draw edge tiles that appear as pan decelerates.
          While actively panning, world.x/y moves instantly — no redraw needed.
          We rely on the panEndTimer + touchend to trigger redraws instead. ── */
    app.ticker.add(() => {
      const empty = !lastBoundsRef.current;
      if (empty) redraw(); // initial draw only
    });
    ...
    const onTS = e => {
      ...
      if (e.touches.length === 1) {
        ...
        // Clear any pending pan-end timer so ticker doesn't fire a stale redraw
        // right as the new gesture starts (causes the visible stutter)
        if (panEndTimer.current) { clearTimeout(panEndTimer.current); panEndTimer.current = null; }
        isPanning.current = true; // suppress ticker redraws immediately on touch
