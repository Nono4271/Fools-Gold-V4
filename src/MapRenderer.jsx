...
export const MapRenderer = memo(forwardRef(function MapRenderer({ tiles, cmds, selKey, mode, mvCmd, reinMarchesRef, panRef: panRefProp, zoomRef: zoomRefProp, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }, ref) {
  ...
  useEffect(() => {
    ...
    function redraw(force = false) {
      ...
      // Only repaint props when tiles changed or viewport revealed new tiles
      if (z >= 1.0 && (propsDirty || boundsChanged)) {
    ...
    const onTM = e => {
      ...
      if (e.touches.length === 1) {
        ...
        if (tDidDrag.current) {
          ...
          // Throttled redraw during pan so newly-revealed tiles render while dragging.
          // Without this, only the initial viewport's tiles are visible until touchend.
