...
export const MapRenderer = memo(forwardRef(function MapRenderer({ tiles, cmds, selKey, mode, mvCmd, reinMarchesRef, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }, ref) {
  ...
  const tOrigin     = useRef({ x: 0, y: 0 }); // touchstart position for total-displacement drag detection
  ...
  const panGenRef   = useRef(0);       // increments each time a pan starts; redraw aborts if it changed mid-draw
  ...
  useEffect(() => {
    ...
    function redraw(force = false) {
      ...
      // Snapshot pan generation before the draw. If a new pan starts while
      // drawAllTiles is executing (it can take 10-20ms for large viewports),
      // we discard the result so stale tiles never flash on screen.
      const genAtDrawStart = panGenRef.current;
      ...
      // If a new pan started while we were drawing, throw away this frame.
      if (panGenRef.current !== genAtDrawStart) {
        lastBoundsRef.current = null; // re-draw once pan settles
        return;
      }
    ...
    const onTS = e => {
      ...
      if (e.touches.length === 1) {
        ...
        tOrigin.current    = { x: e.touches[0].clientX, y: e.touches[0].clientY };
    ...
    const onTM = e => {
      ...
      if (e.touches.length === 1) {
        ...
        // Measure total displacement from touchstart origin (not per-frame delta)
        // so a stationary tap that barely moves doesn't get misclassified as a drag.
        const totalDx = e.touches[0].clientX - tOrigin.current.x;
        const totalDy = e.touches[0].clientY - tOrigin.current.y;
        if (Math.abs(totalDx) + Math.abs(totalDy) > 10) tDidDrag.current = true;
        tDragFrom.current = { x: e.touches[0].clientX, y: e.touches[0].clientY };
        if (tDidDrag.current) {
          if (!isPanning.current) { isPanning.current = true; panGenRef.current++; }
