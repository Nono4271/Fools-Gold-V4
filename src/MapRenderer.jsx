...
function drawAllTiles(gfx, tiles, rMin, rMax, cMin, cMax, selKey, mode, cByTile, mvCmdUid, zoom = 1) {
  ...
  for (let d = dMin; d <= dMax; d++) {
    ...
    for (let c = cLo; c <= cHi; c++) {
      ...
      const { terrain, owner, isHQ, isWin, isKeep } = tile;
      const key = `${c},${r}`;
...
export function MapRenderer({ tiles, cmds, reinMarches, selKey, mode, mvCmd, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }) {
  ...
  // ONE Graphics object per visual layer — not per tile
  ...
  const reinRef  = useRef(reinMarches);
  ...
  useEffect(() => { reinRef.current  = reinMarches; }, [reinMarches]);
  ...
  const panEndTimer = useRef(null);    // fires redraw shortly after pan ends
  ...
  useEffect(() => {
    ...
    let app;
    try {
      app = new PIXI.Application({ width:w, height:h, backgroundColor:0x091e32,
        antialias:false, resolution:Math.min(window.devicePixelRatio||1,2), autoDensity:true });
    } catch(_) {
      try { app = new PIXI.Application({ width:w, height:h, backgroundColor:0x091e32, forceCanvas:true }); }
    ...
    // Create one Graphics per layer
    const tileGfx = new PIXI.Graphics(); world.addChild(tileGfx); tileGfxRef.current = tileGfx;
    const propsGfx = new PIXI.Graphics(); world.addChild(propsGfx); propsGfxRef.current = propsGfx;
    ...
    function redraw(force = false) {
      ...
      // Always redraw tiles
      drawAllTiles(tileGfxRef.current, curTiles, b.rMin, b.rMax, b.cMin, b.cMax,
        selRef.current, modeRef.current, cByTile, mvCmdRef.current?.uid, z);

      // Props are expensive — only show when zoomed in enough to see them
      const z = zoomRef.current;
      if (z >= 1.0) {
        drawAllProps(propsGfxRef.current, curTiles, b.rMin, b.rMax, b.cMin, b.cMax);
      } else {
        propsGfxRef.current?.clear();
      }
    }
    ...
    redrawRef.current = { redraw, redrawOverlays };

    // Initial draw
    redraw(true);
    redrawOverlays();

    /* ── Ticker: skip tile redraws while panning — world moves instantly via x/y.
          Redraw fires when pan stops, or immediately if nothing is drawn yet. ── */
    app.ticker.add(() => {
      const empty = !lastBoundsRef.current;
      if (!isPanning.current || empty) redraw();
    });
    ...
    const onTM = e => {
      ...
      if (e.touches.length === 1) {
        ...
        if (tDidDrag.current) {
          ...
          panRef.current = np;
          world.x = np.x; world.y = np.y; // instant — zero lag
          isPanning.current = true;
          // Schedule edge-tile redraw shortly after movement settles
    ...
    const onTE = e => {
      ...
      if (e.touches.length === 0) {
        ...
        // Pan ended — clear panning flag and redraw edge tiles immediately
        if (panEndTimer.current) clearTimeout(panEndTimer.current);
        isPanning.current = false;
        redraw();
        tDidDrag.current = false;
  ...
  /* ── Sync pan from props (HQ center button, external navigation) ── */
  useEffect(() => {
  ...
  /* ── Redraw on game state changes (selection, battle, march, etc.) ── */
  useEffect(() => {
    tilesRef.current = tiles; // ensure ref is current before redraw
    lastBoundsRef.current = null;
    isPanning.current = false; // never block redraws on state changes
