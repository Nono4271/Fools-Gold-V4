...
export const MapRenderer = memo(forwardRef(function MapRenderer({ tiles, cmds, selKey, mode, mvCmd, reinMarchesRef, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }, ref) {
  ...
  const tFingerDown = useRef(false); // true only while finger is on screen
  ...
  useEffect(() => {
    ...
    const onTM = e => {
      e.preventDefault();
      if (!tFingerDown.current) return; // finger already lifted — drop late events
      if (e.touches.length === 1) {
    ...
    const onTE = e => {
      ...
      if (e.touches.length === 0) {
        tFingerDown.current = false; // immediately block any queued touchmove events
        if (!tDidDrag.current) {
          ...
          if (key) {
            selGfx.clear(); // clear previous selection outline immediately
            selRef.current = key;
