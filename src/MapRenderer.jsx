...
function drawAllTiles(gfx, tiles, rMin, rMax, cMin, cMax, selKey, mode, cByTile, mvCmdUid) {
  gfx.clear();
  ...
  useEffect(() => {
    ...
    function redraw(force = false) {
      ...
      if (!force && last &&
          ...
          b.cMin===last.cMin && b.cMax===last.cMax) return;
      ...
      drawAllTiles(tileGfxRef.current, curTiles, b.rMin, b.rMax, b.cMin, b.cMax,
        selRef.current, modeRef.current, cByTile, mvCmdRef.current?.uid);
    ...
    /* ── Ticker: cheap bounds check every frame, only redraws when viewport changes ── */
    ...
    const onTM = e => {
      ...
      if (e.touches.length === 1) {
        ...
        if (tDidDrag.current) {
          ...
          panRef.current = np;
          world.x = np.x; world.y = np.y; // instant — no React state update
          // ticker's redraw() call will handle spawning edge tiles next frame
        }
