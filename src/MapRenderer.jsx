...
export const MapRenderer = memo(forwardRef(function MapRenderer({ tiles, cmds, selKey, mode, mvCmd, reinMarchesRef, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }, ref) {
  ...
  useEffect(() => {
    ...
    // Single-buffer tile layers — clear + redraw happen in same JS call,
    // PIXI only renders once per frame so there is never a blank frame between them.
    const tileGfx  = new PIXI.Graphics(); world.addChild(tileGfx);  tileFrontRef.current = tileGfx;
    const propsGfx = new PIXI.Graphics(); world.addChild(propsGfx); propsFrontRef.current = propsGfx;
    // Keep back refs pointing at the same object — redraw() uses front refs only now
    tileBackRef.current  = tileGfx;
    propsBackRef.current = propsGfx;
    ...
    function redraw(force = false) {
      ...
      // clear() + draw() happen in the same JS execution — PIXI renders once
      // per frame at the end of the ticker, so there is never a blank frame.
    ...
    /* ── Ticker: redraws tiles when viewport changes or state invalidated ── */
    app.ticker.add(() => {
      if (isPanning.current) return;
      redraw();
      // After a touch ends we do exactly one redraw to fill edge tiles, then
      // go back to the normal bounds-check path (which skips if nothing changed).
      if (pendingPanEnd) { pendingPanEnd = false; isPanning.current = false; }
    });
    ...
    /* ── Touch ── */
    let pendingPanEnd = false; // true after touchend, cleared by next ticker frame
    const onTS = e => {
      e.preventDefault();
      if (e.touches.length === 1) {
        ...
        pendingPanEnd     = false;
        isPanning.current = true; // suppress ticker during the whole touch — tap or pan
    ...
    const onTE = e => {
      ...
      if (e.touches.length === 0) {
        ...
        tDidDrag.current  = false;
        pendingPanEnd     = true;  // ticker will do one redraw then clear this
        lastBoundsRef.current = null;
  ...
  useEffect(() => {
    lastBoundsRef.current = null; // ticker redraws tiles next frame automatically
    if (panEndTimer.current) { clearTimeout(panEndTimer.current); panEndTimer.current = null; }
    redrawRef.current?.redrawOverlays(); // overlays are cheap, sync is fine
