...
export function MapRenderer({ tiles, cmds, reinMarches, selKey, mode, mvCmd, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }) {
  ...
  useEffect(() => {
    ...
    const TILE_BUF = 4; // generous buffer for isometric offset

    // Screen → world coords (unzoomed)
    const wxL = (-pan.x) / zoom;
    const wxR = (-pan.x + sw) / zoom;
    const wyT = (-pan.y) / zoom;
    const wyB = (-pan.y + sh) / zoom;

    // Row range from world y
    const rMin = Math.max(0,        Math.floor((wyT - TOP_PAD) / (TH / 2)) - TILE_BUF);
    const rMax = Math.min(ROWS - 1, Math.ceil( (wyB - TOP_PAD) / (TH / 2)) + TILE_BUF);

    // Col range — isometric x offset per row is TW/2 or TW, so use generous bounds
    // isoXY: cx = c * TW + (r%2===0 ? TW/2 : TW)
    // Solving for c: c = (wx - offset) / TW  where offset is TW/2 or TW
    const cMin = Math.max(0,        Math.floor(wxL / TW) - TILE_BUF - 1);
    const cMax = Math.min(COLS - 1, Math.ceil( wxR / TW) + TILE_BUF + 1);
    ...
    /* gap fills and blends — only redraw visible viewport */
    if (needsFullRedraw.current || didBlend) {
      if (gapGfxRef.current)   drawGapFills(gapGfxRef.current, tiles, rMin, rMax, cMin, cMax);
      if (blendGfxRef.current) drawBlend(blendGfxRef.current, tiles, rMin, rMax, cMin, cMax);
