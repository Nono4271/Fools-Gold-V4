...
function buildKeepLayer(keepCont, tiles, selKey, onKeepClick, PIXI) {
  ...
  while (keepCont.children.length) {
    const c = keepCont.children[0];
    keepCont.removeChild(c);
  ...
  for (const reg of KEEP_REGION_LIST) {
    ...
    const elev = 8;
    // Keep graphic origin — raised above tile surface
    const by   = worldCY - elev - 10;

    // ── Footprint boundary — outer edge of the full 11×11 tile footprint ──
    // Square tile-space footprint maps to a 4-point iso diamond.
    // Verified from tile corner math at elev=0:
    //   top:    tile(-5,-5) top corner  → (bx,      worldCY - 160)
    //   right:  tile( 5,-5) right corner→ (bx+352,  worldCY +  16)
    //   bottom: tile( 5, 5) bottom corner→(bx,      worldCY + 192)
    //   left:   tile(-5, 5) left corner → (bx-352,  worldCY +  16)
    const gy = worldCY; // no offset — flat ground tiles use cy directly
    const FOOTPRINT = [
       bx,        gy - 160,  // top
       bx + 352,  gy +  16,  // right
       bx,        gy + 192,  // bottom
    ...
    // ── Selection / hover outline drawn on the footprint border ───────────
    if (isSelected) {
      ...
      outlineGfx.lineStyle(0);
      keepCont.addChild(outlineGfx);
    }

    // ── Keep graphic ───────────────────────────────────────────────────────
    const gfx = new PIXI.Graphics();
    drawKeepGfx(gfx, bx, by, owner, reg.isWin, false); // selection handled by outline above
    keepCont.addChild(gfx);

    // ── Full-footprint hit area ────────────────────────────────────────────
    // Use hitArea instead of a drawn polygon — works reliably across Pixi versions
    // regardless of fill alpha. Covers the entire footprint diamond.
    const hit = new PIXI.Graphics();
    hit.beginFill(0xffffff, 0.001);
    hit.drawRect(bx - 352, gy - 160, 704, 352); // bounding rect for display object
    hit.endFill();
  ...
  useEffect(() => {
    ...
    const onTE = e => {
      ...
      if (e.touches.length === 0) {
        ...
        tDidDrag.current = false;
        // Clear throttle timer and flush final pan position to parent
        if (panNotifyTimer.current) {
