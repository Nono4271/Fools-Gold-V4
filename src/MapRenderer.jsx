...
export const MapRenderer = memo(forwardRef(function MapRenderer({ tiles, cmds, selKey, mode, mvCmd, reinMarchesRef, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }, ref) {
      ...
      if (e.touches.length === 0) {
        if (!tDidDrag.current) {
          ...
          if (key) {
            ...
            console.log('[tap] onTileClick took', (performance.now()-t0).toFixed(1), 'ms');
          }
        } else {
          console.log('[pan end] world.x:', world.x.toFixed(1), 'panRef:', panRef.current.x.toFixed(1));
        }
  ...
  useEffect(() => {
    console.log('[panSt]', panSt.x.toFixed(1), panSt.y.toFixed(1), '| world was:', worldRef.current?.x?.toFixed(1));
