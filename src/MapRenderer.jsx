...
export const MapRenderer = memo(forwardRef(function MapRenderer({ tiles, cmds, selKey, mode, mvCmd, reinMarchesRef, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }, ref) {
  ...
  // panSt is only set at map spawn. All pan during gameplay goes through
  // panRef via native touch/mouse events and never touches panSt.
  // The teleport() imperative method handles HQ-center and WorldMap jumps directly.
  // This effect is intentionally left empty — removing it entirely prevents
  // the snap that occurred because panSt (stuck at spawn coords) would overwrite
  // world.x/y whenever any React re-render triggered this effect.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  // (no effect needed — teleport() handles all programmatic pan)
