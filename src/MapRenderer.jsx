...
export const MapRenderer = memo(forwardRef(function MapRenderer({ tiles, cmds, selKey, mode, mvCmd, reinMarchesRef, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }, ref) {
  ...
  useEffect(() => {
    ...
    /* ── Mouse (desktop) ── */
    let mouseDrag = false, mouseDidDrag = false;
    let mouseDragFrom = { x: 0, y: 0 };

    const onMouseDown = e => {
      if (e.button !== 0) return;
      mouseDrag = true; mouseDidDrag = false;
      mouseDragFrom = { x: e.clientX, y: e.clientY };
      e.preventDefault();
    };
    const onMouseMove = e => {
      if (!mouseDrag) return;
      const dx = e.clientX - mouseDragFrom.x;
      const dy = e.clientY - mouseDragFrom.y;
      if (Math.abs(dx) + Math.abs(dy) > 4) mouseDidDrag = true;
      mouseDragFrom = { x: e.clientX, y: e.clientY };
      if (mouseDidDrag) {
        const np = clampPan(panRef.current.x+dx, panRef.current.y+dy, zoomRef.current);
        panRef.current = np;
    ...
    const onMouseUp = e => {
      ...
      const wasDrag = mouseDidDrag;
      mouseDrag = false; mouseDidDrag = false;
  ...
  useEffect(() => {
    lastBoundsRef.current = null; // causes ticker to redraw tiles next frame
