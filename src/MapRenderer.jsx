...
export const MapRenderer = memo(forwardRef(function MapRenderer({ tiles, cmds, selKey, mode, mvCmd, reinMarchesRef, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }, ref) {
  ...
  useEffect(() => {
    // selKey change: just update the selection outline on its own layer.
    // Do NOT null lastBoundsRef — that triggers a full drawAllTiles which
    // is the cause of the visible stutter on every tap.
    redrawRef.current?.redrawSelection(selKey);
    redrawRef.current?.redrawOverlays();
  }, [selKey]);

  useEffect(() => {
    // mode/mvCmd change affects tile highlight colours — need full redraw
    lastBoundsRef.current = null;
  ...
  useEffect(() => {
    ...
    // Don't null lastBoundsRef here — march ticks update cmds every 100ms and
    // forcing a full tile redraw that often causes the alignment snap.
    // The ticker redraws tiles when bounds change (pan/zoom), overlays handle
    // commander icons and march lines separately via redrawOverlays.
    redrawRef.current?.redrawOverlays();
  }, [cmds]);
