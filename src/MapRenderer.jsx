...
export const MapRenderer = forwardRef(function MapRenderer({ tiles, cmds, reinMarches, selKey, mode, mvCmd, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }, ref) {
    ...
    const onTE = e => {
      ...
      if (e.touches.length === 0) {
        if (!tDidDrag.current) {
          ...
          if (key) {
            selRef.current = key; // update immediately so redraw shows selection at once
            lastBoundsRef.current = null;
            redrawRef.current?.redraw(true);
            onTileClickRef.current(key, e); // then notify React
          }
        }
        isPanning.current = false;
        tDidDrag.current = false;
        lastBoundsRef.current = null;
        redrawRef.current?.redraw(true); // immediate redraw at final pan position — no snap
  ...
  useEffect(() => {
    ...
    lastBoundsRef.current = null; // signals ticker to redraw
  }, [tiles]);

  useEffect(() => {
    cmdsRef.current = cmds;
    reinRef.current = reinMarches;
    lastBoundsRef.current = null;
  }, [cmds, reinMarches]);
