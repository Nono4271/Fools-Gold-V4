...
export function MapRenderer({ tiles, cmds, reinMarches, selKey, mode, mvCmd, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }) {
  ...
  useEffect(() => {
    ...
    const onWheel = e => {
      ...
      if (levels[ni] !== zoomRef.current) {
        ...
        lastBoundsRef.current = null;
        redraw(true);           // draw new tiles at new zoom FIRST
        world.scale.set(levels[ni]); // THEN scale — no gap flash
  ...
  useEffect(() => {
    ...
    lastBoundsRef.current = null;
    redrawRef.current?.redraw(true);  // draw at new zoom first
    if (worldRef.current) worldRef.current.scale.set(zoom); // then scale
  }, [zoom]);
