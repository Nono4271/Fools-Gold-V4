...
export const MapRenderer = memo(forwardRef(function MapRenderer({ tiles, cmds, selKey, mode, mvCmd, reinMarchesRef, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }, ref) {
  ...
  useEffect(() => {
    ...
    function redraw(force = false) {
      ...
      // Defer front.clear() to a microtask so PIXI renders the new back buffer
      // before we wipe the old front — prevents a 1-frame flash of blank tiles.
      back.visible  = true;
      front.visible = false;
      tileBackRef.current  = front;
      tileFrontRef.current = back;
      Promise.resolve().then(() => front.clear());
    ...
    const onTM = e => {
      ...
      if (e.touches.length === 1) {
        ...
        if (tDidDrag.current) {
          ...
          isPanning.current = true;
          // Mirror mouse-drag behaviour: schedule a redraw shortly after panning stops
          // so edge tiles that scroll into view get drawn even while isPanning suppresses the ticker.
