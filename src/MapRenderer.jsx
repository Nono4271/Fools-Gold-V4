...
export function MapRenderer({ tiles, cmds, reinMarches, selKey, mode, mvCmd, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }) {
  ...
  const cullAndSpawnRef = useRef(null);
  ...
  useEffect(() => {
    ...
    /* ── Shared cull+spawn — called every frame during pan/zoom ── */
    function cullAndSpawn() {
      ...
      // Evict tiles outside buffer zone
      ...
      // Spawn / redraw — iterate diagonals (d = c+r) ascending so addChild
      // order naturally matches painter order. No sortableChildren needed.
      ...
      const dMin  = cMin + rMin;
      const dMax  = cMax + rMax;
    ...
    cullAndSpawnRef.current = cullAndSpawn;

    /* ── Per-frame ticker ── */
    app.ticker.add(() => {
      // Run cull every frame while panning (camera must feel instant)
      // and for a few frames after any change settles.
      if (isPanning.current || needsFullRedraw.current || panCooldown.current > 0) {
        if (!isPanning.current) panCooldown.current--;
    ...
    const onWheel = e => {
      ...
      if (levels[ni] !== zoomRef.current) {
        ...
        if (cullAndSpawnRef.current) cullAndSpawnRef.current();
  ...
  useEffect(() => {
    ...
    // Call cull immediately — don't wait for next ticker frame
    if (cullAndSpawnRef.current) cullAndSpawnRef.current();
    panCooldown.current = 8;
  }, [zoom]);
