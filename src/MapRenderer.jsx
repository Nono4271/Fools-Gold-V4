...
export const MapRenderer = forwardRef(function MapRenderer({ tiles, cmds, reinMarches, selKey, mode, mvCmd, panSt, zoom, ZOOM_LEVELS, onTileClick, onPanChange, onZoomChange }, ref) {
  ...
  useEffect(() => {
    ...
    /* ── Ticker: redraws edge tiles whenever the viewport shifts.
          While panning, world.x/y moves instantly (zero lag).
          The ticker fires the tile redraw ~1 frame after pan settles. ── */
    app.ticker.add(() => {
    ...
    const panChangeTimer = { current: null };
    ...
    const onTS = e => {
      ...
      if (e.touches.length === 1) {
        ...
        // Cancel any pending panChange notification — user started a new pan
        if (panChangeTimer.current) { clearTimeout(panChangeTimer.current); panChangeTimer.current = null; }
    ...
    const onTE = e => {
      ...
      if (e.touches.length === 0) {
        ...
        isPanning.current = false; // ticker resumes redraws immediately
        tDidDrag.current = false;
        // Debounce: only notify React after 300ms of no new pans
        // This prevents setPanSt → re-render → redraw() from firing mid-swipe
        if (panChangeTimer.current) clearTimeout(panChangeTimer.current);
        panChangeTimer.current = setTimeout(() => {
          onPanChangeRef.current(panRef.current);
          panChangeTimer.current = null;
        }, 300);
      }
    };
  ...
  useEffect(() => {
    ...
    lastBoundsRef.current = null;
    // Don't call redraw here — ticker picks it up next frame (~16ms)
    // Calling redraw() here + ticker = double redraw = visible stutter
  }, [panSt]);
